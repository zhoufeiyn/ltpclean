from cleanedltp import dataloader
import random
import torch
...
def process_npdata(np_data, horizon_len, start_idx, device):
    ...
    dict_data = dataloader.split_data_mario(epi_data, horizon_len, device)
    batch = {}
...
def get_model():
    model = dataloader.read_model(model_name, model_path, action_space, device)
    return model
