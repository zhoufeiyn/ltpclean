...
def train():
    ...
    data_save_epoch = cfg.loss_log_iter
    ...
    print(
        f"dataset loaded: {total_samples} samples, construct {num_videos} complete video sequences, each video has {num_frames} frames, construct {num_videos // batch_size} batches, each batch has {batch_size} videos")
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, total_samples, batch_size * num_frames):
            ...
            if i + batch_size * num_frames > total_samples:
                print(
                    f"⚠️ jump to next batch: need {batch_size * num_frames} samples, but only {total_samples - i} samples left")
                break
            ...
            for batch_idx in range(batch_size):
                ...
                if end_idx > total_samples:
                    print(
                        f"⚠️ jump to next batch: start_idx={start_idx}, end_idx={end_idx}, total_samples={total_samples}")
                    break
