...
def process_npdata(np_data, horizon_len, start_idx, device):
    ...
    dict_data = utils2.split_data_mario(epi_data, horizon_len, device)
    batch = {}
...
def get_model():
    model = utils2.read_model(model_name, model_path, action_space, device)


    return model

np_data = utils2.read_file(file_path, data_type)
