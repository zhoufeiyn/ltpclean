...
class SelfAttention2d(nn.Module):
    def __init__(self, in_channels: int, n_head: int = ATTN_NUM_HEAD) -> None:
        super().__init__()
...
class ResBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, cond_channels: int, attn: bool,
        ...
        self.attn = SelfAttention2d(out_channels,attn_head_num) if attn else nn.Identity()
...
class ResBlocks(nn.Module):
    def __init__(
        ...
        self.resblocks = nn.ModuleList(
            [
                ResBlock(in_ch, out_ch, cond_channels, attn, attn_head_num)
